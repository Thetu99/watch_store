!function (a) {
  ...
  a.prototype = {
    init: function () { beta.initInstragram(), beta.initFilter(), beta.initCalendar(), beta.initRevolutionSlider(), beta.initRoundabout(), beta.initSlider(), beta.initHistorySlider(), beta.initCircliful(), beta.initMedia(), beta.initTooltip(), beta.initProgressBar(), beta.initColorbox(), beta.onCart(), beta.setSelect(), beta.animateBanners(), beta.absFullwidth(), beta.filterItems(), beta.accordionHandler() }, initInstragram: function () {
      /* dug({ endpoint: "https://api.instagram.com/v1/users/1140781/media/recent/?client_id=01a35e4dd4bf4d2681e48c88d006636e&count=6", target: jQuery("#beta-instagram-feed")[0], cacheExpire: 3600, template: '<a href="{{link}}" target="_blank"><img src="{{images.low_resolution.url}}"></a>{{/data}}' }) */
    }, initFilter: function () {
      jQuery.fn.slider && (jQuery("#price-filter-range").slider({ range: !0, min: 0, max: 1e3, values: [0, 300], slide: function (a, b) { jQuery("#price-filter-amount").text("$" + b.values[0] + " - $" + b.values[1]) } }), jQuery("#price-filter-amount").text("$" + jQuery("#price-filter-range").slider("values", 0) + " - $" + jQuery("#price-filter-range").slider("values", 1)))
      ...
      if (jQuery.fn.circliful) {
        var a = window.matchMedia("(max-width: 1200px)"), b = window.matchMedia("(max-width: 993px)"), c = window.matchMedia("(max-width: 768px)");
      ...
      jQuery(".beta-slider").each(function (a, b) {
        var c = jQuery(b), d = c.find(".beta-slider-items"), e = c.find(".beta-arrow-right"), f = c.find(".beta-arrow-left"), g = c.find(".beta-pager"), h = c.find(".beta-pager-gallery"), i = { auto: !0, autoHover: !0, controls: !1, pager: !1 };
      ...
      jQuery("select[name='languages']").betaSelect({ text: "Language:", value: "English" }), jQuery("select[name='currency']").betaSelect({ text: "Currency:", value: "USD" }), jQuery(".beta-select-primary").betaSelect({ value: "Select", "class": "beta-select-primary" })
    }, onCart: function () {
  ...
  jQuery(function () {
    new a;
    "undefined" != typeof BetaUtils && BetaUtils.setMap(jQuery("#beta-map")[0], { latitude: 40.722803, longitude: -74.00882, zoom: 10, content: "BetaDesign", title: "Beta Design" }), b.init(), jQuery(".shipping-calculator-button").on("click", function () {
